...
test.describe("S1.4 Rate limiting", () => {
  test("API write: 20 pass, 21st returns 429", async ({ context }) => {
    ...
    const passResults = await Promise.all(
      Array.from({ length: 20 }, () => context.request.post(url)),
    );
    ...
    const blocked = await context.request.post(url);
    expect(blocked.status()).toBe(429);
    expect(blocked.headers()["retry-after"]).toBeDefined();
  });

  test("API read: 60 pass, 61st returns 429", async ({ context }) => {
    await setSessionCookie(context, user.signedSessionId);
    const url = "/api/disponibilidad";

    // Send 60 parallel GETs (should all pass)
    const passResults = await Promise.all(
      Array.from({ length: 60 }, () => context.request.get(url)),
    );
    for (const res of passResults) {
    ...
    const blocked = await context.request.get(url);
    expect(blocked.status()).toBe(429);
    expect(blocked.headers()["retry-after"]).toBeDefined();
  });
});
