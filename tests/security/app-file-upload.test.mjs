...
// 2. File vacío → null
assert.equal(
  await processAppFileUpload(makeFile("test.zip", "", "application/zip"), null),
  null,
  "empty file → null",
);
...
try {
  ...
  assert.ok(
    e.message.includes("tipo de archivo"),
...
try {
  const big = new File([Buffer.alloc(101 * 1024 * 1024 + 1)], "big.zip", {
