...
async function runTest() {
  ...
  const isVariantDisabled = await page
    .locator("#filter-os-variant")
    .getAttribute("disabled");
  assert.equal(
    isVariantDisabled,
    null,
    "El selector de variantes debería estar habilitado al elegir Windows 10",
  );
  ...
  const hostnameHeaderButton = page.locator(
    "#table-terminales button[data-table-sort-key='hostname']",
  );
  ...
  assert.ok(
    hasSortCall,
    "Debería haberse realizado una llamada a la API con ordenamiento por hostname",
  );
  ...
  await page
    .locator("#terminal-search")
    .pressSequentially("zabw", { delay: 100 });
  ...
  assert.ok(
    searchRows.length > 0,
    "Debería haber al menos un resultado al buscar por infijo 'zabw'",
  );
  const firstRowText = await searchRows[0].innerText();
  assert.ok(
    firstRowText.includes("1428ZABW0001") || firstRowText.includes("ZABW"),
    "El resultado debería incluir la terminal con ZABW",
  );
  ...
  await page
    .locator("#terminal-search")
    .pressSequentially("308", { delay: 100 });
  ...
  assert.ok(
    searchRows308.length > 0,
    "Debería haber al menos un resultado al buscar por infijo '308'",
  );
  const text308 = await searchRows308[0].innerText();
  assert.ok(
    text308.includes("A0000308"),
    "Debería encontrar la terminal A0000308",
  );
