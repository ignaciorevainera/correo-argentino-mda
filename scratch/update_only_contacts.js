...
const csvContactosPath = path.join(
  __dirname,
...
rowsContactos.slice(1).forEach((row) => {
  ...
  const names = nombreStr
    ...
    .map((s) => s.trim())
    .filter((s) => s);
  const phones = telefonoStr
    .split("\n")
    .map((s) => s.trim())
    .filter((s) => s);
  const times = horarioStr
    .split("\n")
    .map((s) => s.trim())
    .filter((s) => s);
...
const blockRegex =
  /(code:\s*"(I\d{4}|CORR\d{1})",[\s\S]*?lng:\s*-?\d+(?:\.\d+)?,\s*\n\s*)email:\s*".*?",\s*\n\s*notes:\s*".*?",\s*\n\s*contacts:\s*\[[\s\S]*?\],(?=\s*\n\s*assets:)/g;
...
tsContent = tsContent.replace(blockRegex, (match, beforeBlock, code) => {
  ...
  let emailStr = fixAccents(cData.email)
    .replace(/"/g, '\\"')
    .replace(/\n/g, "\\n")
    .replace(/\r/g, "");
  let notesStr = fixAccents(cData.notes)
    .replace(/"/g, '\\"')
    .replace(/\n/g, "\\n")
    .replace(/\r/g, "");
  ...
  cData.contacts.forEach((c) => {
    ...
    let cTimeSlot = fixAccents(c.timeSlot)
      .replace(/"/g, '\\"')
      .replace(/\n/g, " ");
