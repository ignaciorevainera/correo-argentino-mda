...
- **Edición de usuario (modal único):** `/admin/usuarios` usa un solo modal "Editar usuario" que postea `action=update-user` y edita en una única transacción síncrona: rol, mesa de ayuda (canónica desde `mesas.name`), `users.username`, `agents.name` y los 4 flags de participación. Renombrar el nombre visible propaga `schedules.agentName`. La fila `agents` se resuelve por `lower(username)` y se crea si falta (self-heal). Colisiones de `users.username`/`agents.name` se detectan dentro de la tx con mensaje específico. Mesa no participativa ⇒ flags a false; supervisor ⇒ `enCronograma=false`. La contraseña se blanquea con el CTA separado (`reset-password`). Snapshot leído dentro de la tx (sin TOCTOU).
- **Vínculo de horarios por ID:** `schedules.agentId` → `agents.id` → `agents.userId` → `users.id` (ambas FK nullable, `onDelete: set null`). `schedules.agentName` se mantiene como columna de compatibilidad/display (Plan B la migra a lectores y luego se dropea). Los flags de participación (`enCronograma`, etc.) son solo visibilidad, nunca vínculo. Backfill: `scripts/migrate-schedule-links.mts` (dry-run por defecto, `--apply` con backup WAL-safe). `agents` es el **perfil de operador** (no solo un alias de `users`): guarda configuración de horarios, ubicación, notas y capacidades; puede existir sin usuario de portal (`username` NULL). Nunca correr --apply en prod sin dry-run previo revisado y backup verificado (restringido a DB dev salvo autorización explícita).
- **Lectores por ID (fase B1):** los 4 lectores de `schedules` (cronograma GET, `attendance`, `disponibilidad`, `asistencia/operador`) matchean por `agentId` y caen a `agentName` solo si `agentId IS NULL` (transición). La API de cronograma (POST/PUT `edits`/`weeklySchedules`) acepta `agentId` opcional (normalizado a entero, 400 si inválido; respuesta con `saved`/`skipped` del conteo de `edits` — `weeklySchedules` omite en silencio); sin él opera por nombre (cliente actual). La propagación de rename y el drop de `agentName` quedan para la fase B2.
- **`/admin/permisos`:** solo mesas — sync InvGate (CSRF + rate-limit, devuelve `affectedUsers` con banner de reasignacion en `/admin/usuarios`) + resumen read-only de visibilidad. El resumen evalua **ambas capas** (`isSectionVisibleSync` ∧ `hasPermission`) para un rol `agent`, asi que refleja el acceso real (p.ej. `/admin` sale ✗ para agentes MDA TI).
