...
- **Banners de `/admin/usuarios`:** tres estados derivados de la DB, todos fail-closed — (a) usuario **sin mesa** (`helpdeskId` NULL) o **mesa huerfana** (`helpdeskId` apunta a un `invgateId` inexistente), (b) usuario con **mesa inactiva** (`mesas.active = false`). Ambos listados con cap de 20 + contador de restantes.
- **Participaciones:** gating por mesa **canonica**: el selector de participaciones (y el alta/change-role) resuelve la mesa via `LEFT JOIN users→mesas` (`mesas.name`) con fallback al denormalizado `users.helpdeskName` solo si el join no resuelve (usuario huerfano); luego `mesaHasParticipaciones`. Solo mesas en `PARTICIPATION_HELPDESK_NAMES` (hoy MDA TI). El supervisor no figura: `enCronograma` forzado a `false` server-side (`normalizeRole`).
