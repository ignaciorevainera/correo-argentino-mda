...
class AttendanceStore {
  ...
  public updateField(rowId: string, field: keyof AttendanceRecord, value: any) {
    ...
    if (
      field === "entradaReal" ||
      field === "cumplimiento" ||
      field === "horarioEstipulado"
    ) {
      const curEntrada =
        edit.entradaReal !== undefined ? edit.entradaReal : record.entradaReal;
      const curHorarioEstipulado =
        edit.horarioEstipulado !== undefined
          ? edit.horarioEstipulado
          : record.horarioEstipulado;
      if (field === "cumplimiento") {
        if (value === "") {
          edit.cumplimientoForzado = false;
          edit.cumplimiento = calculateCompliance(
            curEntrada,
            curHorarioEstipulado,
          );
        } else {
        ...
        const curForced =
          edit.cumplimientoForzado !== undefined
            ? edit.cumplimientoForzado
            : record.cumplimientoForzado;
        if (!curForced) {
          edit.cumplimiento = calculateCompliance(
            curEntrada,
            curHorarioEstipulado,
