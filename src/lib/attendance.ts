...
export async function getAttendanceData(startDate: string, endDate: string) {
  ...
  if (months.length > 0) {
    const dbScheduled = await db
      ...
      .from(schedules)
      .where(inArray(sql<string>`substr(${schedules.date}, 1, 7)`, months))
  ...
  const rotationConfigsByMonth: Record<
    string,
    typeof saturdayRotationConfig.$inferSelect
  > = {};
  for (const m of months) {
  ...
  const allDbAgentSaturdayGroups =
    months.length > 0
      ? await db
          .select()
  ...
  const resolvedAgentSaturdayConfig: Record<
    string,
    { group: string | null; horarioSat: string | null }
  > = {};
  ...
  for (const dateStr of dates) {
    ...
    dbAgents.forEach((agent) => {
      ...
      if (isSaturday) {
        const satConfig =
          resolvedAgentSaturdayConfig[`${agent.id}_${monthStr}`];
        ...
        if (satGroup) {
          ...
          if (rotConfig) {
            ...
            const groups = rotConfig.rotationOrder
              .split(",")
              .map((g) => g.trim());
      ...
      if (
        ...
        modalidadPlanificada === "Licencia"
      ) {
        horarioEstipulado = "";
      } else if (
        !horarioEstipulado ||
        horarioEstipulado.trim() === "" ||
        horarioEstipulado.trim() === "-"
      ) {
      ...
      if (hasOvertime && overtimeShift) {
        ...
        const cumplimientoForzado =
          actualOvertime?.cumplimientoForzado ?? false;
