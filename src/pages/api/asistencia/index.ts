...
export const GET: APIRoute = async ({ url, locals }) => {
  ...
  try {
    const startDate =
      url.searchParams.get("startDate") || url.searchParams.get("date");
  ...
  try {
    ...
    for (const edit of edits) {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(editDate)) {
        return jsonResponse(
          { error: `Formato de fecha inválido: ${editDate}` },
          400,
        );
      }
      ...
      if (
        ...
        return jsonResponse(
          { error: `Fecha inexistente en el calendario: ${editDate}` },
          400,
        );
      }
      ...
      if (editDate > todayStr) {
        return jsonResponse(
          {
            error: `No se permite registrar asistencia para fechas futuras: ${editDate}`,
          },
          400,
        );
      }
      ...
      if (dateParts[0] !== currentYear) {
        return jsonResponse(
          {
            error: `El registro histórico para el año ${dateParts[0]} está cerrado: ${editDate}`,
          },
          400,
        );
      }
    ...
    await db.transaction((tx) => {
      for (const edit of edits) {
        ...
        if (existing.length > 0) {
          ...
          if (horarioEstipulado !== undefined)
            updateData.horarioEstipulado = horarioEstipulado;
          if (cumplimiento !== undefined)
            updateData.cumplimiento = cumplimiento;
          if (cumplimientoForzado !== undefined)
            updateData.cumplimientoForzado = !!cumplimientoForzado;
          if (motivoLoguin !== undefined)
            updateData.motivoLoguin = motivoLoguin;
          if (detalle !== undefined) updateData.detalle = detalle;

          if (Object.keys(updateData).length > 0) {
            tx.update(operatorAttendance)
              .set(updateData)
              .where(eq(operatorAttendance.id, existing[0].id))
              .run();
          }
        } else {
          tx.insert(operatorAttendance)
            .values({
              agentId,
              date: editDate,
