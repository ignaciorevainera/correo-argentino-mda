...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    const { month, rotationOrder, startDate, startGroup, disabledGroups } =
      body;
    ...
    if (
      ...
      return jsonResponse(
        {
          error:
            "Required fields (startDate, startGroup, rotationOrder) must be strings",
        },
        400,
      );
    }

    const disabledGroupsStr =
      typeof disabledGroups === "string" ? disabledGroups : "";

    const targetMonth =
      typeof month === "string" && month ? month : startDate.slice(0, 7);

    if (!MONTH_REGEX.test(targetMonth)) {
      return jsonResponse(
        { error: "Invalid target month format. Expected YYYY-MM" },
        400,
      );
    }
    ...
    if (isNaN(dateObj.getTime()) || dateObj.getDay() !== 6) {
      return jsonResponse(
        { error: "Start date must be a valid Saturday" },
        400,
      );
    }
