...
function mapStatusText(cellValue: string): string {
  ...
  if (
    val.startsWith("presencial monte grande") ||
    val.startsWith("mg") ||
    val === "monte grande"
  ) {
  ...
  if (
    val.startsWith("presencial parque patricios") ||
    val.startsWith("pp") ||
    val === "parque patricios"
  ) {
  ...
  try {
    ...
    if (isFormatA) {
      ...
      const dateColumns = firstRow
        ...
        .map((col) => ({
          header: normalizeDate(col.rawHeader),
          index: col.index,
        }))
        .filter(
          (col): col is { header: string; index: number } =>
            col.header !== null,
        );
      ...
      if (dateColumns.length === 0) {
        return jsonResponse(
          {
            error:
              "No se detectaron cabeceras de fecha con formato de Excel original ('lunes, 1 de junio de 2026') o YYYY-MM-DD",
          },
          400,
        );
      }
    ...
    return jsonResponse(
      { error: sanitizeError(error, "Error al procesar el archivo CSV") },
      500,
    );
  }
};
