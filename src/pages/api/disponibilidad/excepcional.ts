...
import {
  marcarEstadoExcepcional,
  limpiarEstadoExcepcional,
  ensureHasLock,
...
export const POST: APIRoute = async ({ request, locals }) => {
  const denied = await requireWriteAccess(locals, "asignacion_ag");
  ...
  try {
    ...
    if (!tipo || typeof tipo !== "string") {
      return jsonResponse(
        { success: false, error: "Tipo de excepción inválido" },
        400,
      );
    }

    const result = await marcarEstadoExcepcional(
      agentId,
      tipo,
      motivo,
      tiempoExtra,
