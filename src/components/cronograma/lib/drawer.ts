...
export function openDrawer(opName: string) {
  ...
  if (emailEl)
    emailEl.innerText = op.username
      ? `${op.username}@correoargentino.com.ar`
      : "";

  const minPWeekSelectEl = document.getElementById(
    "drawer-op-min-p-week",
  ) as HTMLSelectElement | null;
  const maxHoSelectEl = document.getElementById(
    "drawer-op-max-ho",
  ) as HTMLSelectElement | null;
  if (minPWeekSelectEl) {
    minPWeekSelectEl.value =
  ...
  Object.values(op.asistencia || {}).forEach((s) => {
    if (s === "Presencial Monte Grande" || s === "Presencial Parque Patricios")
      p++;
  ...
  if (historyList) {
    const sortedDates = Object.keys(op.asistencia || {})
      .sort()
      .reverse();
  ...
  const opWeekly = op.esquema_semanal || {
  ...
  const opWeeklyScheduleTimes = op.esquema_horario || {
    Lunes: "",
    Martes: "",
    Miercoles: "",
    Jueves: "",
  ...
  const opWeeklyBreakInicioTimes = op.esquema_break_inicio || {
    Lunes: "",
    Martes: "",
    Miercoles: "",
    Jueves: "",
  ...
  const opWeeklyBreakFinTimes = op.esquema_break_fin || {
    Lunes: "",
    Martes: "",
    Miercoles: "",
    Jueves: "",
  ...
  setCurrentWeeklyData(
    opWeekly,
    opWeeklyScheduleTimes,
    opWeeklyBreakInicioTimes,
    opWeeklyBreakFinTimes,
  );

  // Reset weekly template select and hide custom actions when opening/switching operator
  const weeklySelect = document.getElementById(
...
export async function saveDrawerOperatorRules(
  minPWeekSelectEl: HTMLSelectElement | null,
  maxHoSelectEl: HTMLSelectElement | null,
) {
  const opName = document.getElementById("drawer-op-name")?.textContent?.trim();
  ...
  if (rulesIndicator) {
    ...
    rulesIndicator.classList.remove(
      "opacity-0",
      "text-success/70",
      "text-error",
    );
