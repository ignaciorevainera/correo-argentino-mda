...
export let currentWeeklyScheme: Record<string, OperatorStatus> = {
  Lunes: OperatorStatus.Franco,
  Martes: OperatorStatus.Franco,
  Miercoles: OperatorStatus.Franco,
  Jueves: OperatorStatus.Franco,
  Viernes: OperatorStatus.Franco,
};
export let currentWeeklyScheduleTimes: Record<string, string> = {
  Lunes: "",
  Martes: "",
  Miercoles: "",
  Jueves: "",
...
export let currentWeeklyBreakInicioTimes: Record<string, string> = {
  Lunes: "",
  Martes: "",
  Miercoles: "",
  Jueves: "",
...
export let currentWeeklyBreakFinTimes: Record<string, string> = {
  Lunes: "",
  Martes: "",
  Miercoles: "",
  Jueves: "",
...
export function populateWeeklyTemplatesDropdown() {
  const select = document.getElementById(
    "weekly-template-select",
  ) as HTMLSelectElement | null;
  if (!select) return;

  const prevVal = select.value;
  let html =
    '<option value="">-- Seleccionar una plantilla o personalizar --</option>';
    ...
    .map((day) => {
      ...
      const optHtml = options
        .map((opt) => {
          ...
          let btnClass =
            "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-base-100 hover:bg-base-200 text-base-content/60 border border-base-300/40";
          if (currentVal === opt) {
            if (opt === "Presencial Monte Grande")
              btnClass =
                "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-amber-500 text-white shadow-md";
            else if (opt === "Presencial Parque Patricios")
              btnClass =
                "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-purple-500 text-white shadow-md";
            else if (opt === "Home Office")
              btnClass =
                "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-secondary text-secondary-content shadow-md";
            else if (opt === "Vacaciones")
              btnClass =
                "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-success text-white shadow-md";
            else if (opt === "Licencia")
              btnClass =
                "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-error text-white shadow-md";
            else if (opt === "Franco")
              btnClass =
                "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-base-300 text-base-content shadow-md";
          }
...
export function handleTemplateSelectChange(name: string) {
  ...
  if (name && templates[name]) {
    ...
    currentWeeklyBreakInicioTimes = {
      ...(templates[name].breaks_inicio || {
        Lunes: "",
        Martes: "",
        Miercoles: "",
        Jueves: "",
    ...
    currentWeeklyBreakFinTimes = {
      ...(templates[name].breaks_fin || {
        Lunes: "",
        Martes: "",
        Miercoles: "",
        Jueves: "",
...
export async function handleDeleteTemplateClick() {
  ...
  const confirmed = await showConfirm(
    `¿Estás seguro de que deseas eliminar la plantilla "${name}"?`,
  );
  if (confirmed) {
...
export async function handleDuplicateTemplateClick() {
  ...
  const newName = await showPrompt(
    "Ingresa el nombre del duplicado:",
    `${name} (Copia)`,
  );
...
export function handleWeeklyFocusOut(target: HTMLInputElement) {
  if (
    target.classList.contains("weekly-break-inicio-input") ||
    target.classList.contains("weekly-break-fin-input")
  ) {
...
export function handleWeeklyDayOptionClick(btn: HTMLButtonElement) {
  ...
  if (btn.parentElement) {
    btn.parentElement
      .querySelectorAll("[data-weekly-option]")
      .forEach((other) => {
        other.className =
          "weekly-day-opt-btn btn btn-xs font-black text-tiny uppercase px-3 py-1.5 h-auto rounded-lg transition-colors duration-200 bg-base-100 hover:bg-base-200 text-base-content/60 border border-base-300/40";
      });
  ...
  let activeClass = "";
  if (option === "Presencial Monte Grande")
    activeClass = "bg-amber-500 text-white shadow-md";
  else if (option === "Presencial Parque Patricios")
    activeClass = "bg-purple-500 text-white shadow-md";
  else if (option === "Home Office")
    activeClass = "bg-secondary text-secondary-content shadow-md";
  else if (option === "Vacaciones")
    activeClass = "bg-success text-white shadow-md";
  else if (option === "Licencia") activeClass = "bg-error text-white shadow-md";
  else if (option === "Franco")
    activeClass = "bg-base-300 text-base-content shadow-md";
...
export async function saveWeeklySchedule(
  ...
  try {
    ...
    if (opIndex !== -1) {
      ...
      if (mode === "all" || mode === "hours") {
        state.cronoData[opIndex].esquema_horario = {
          ...currentWeeklyScheduleTimes,
        };
        state.cronoData[opIndex].esquema_break_inicio = {
          ...currentWeeklyBreakInicioTimes,
        };
        state.cronoData[opIndex].esquema_break_fin = {
          ...currentWeeklyBreakFinTimes,
        };
      }
    }
    ...
    showToast(
      "¡Esquema semanal guardado y aplicado con éxito al mes activo!",
      "success",
    );
    ...
    showToast(
      "Ocurrió un error al intentar guardar el esquema semanal.",
      "error",
    );
  } finally {
    if (saveBtn) {
